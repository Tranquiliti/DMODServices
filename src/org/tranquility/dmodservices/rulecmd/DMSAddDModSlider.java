...
public class DMSAddDModSlider extends BaseCommandPlugin {
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        int maxAddDMods = Math.min(DModManager.MAX_DMODS_FROM_COMBAT - DModManager.getNumDMods(((FleetMemberAPI) localMemory.get(MEM_PICKED_SHIP)).getVariant()), ((List<HullModSpecAPI>) localMemory.get(MEM_ELIGIBLE_DMODS)).size());
        opt.addSelector(RANDOM_DMOD_SELECTOR_TEXT, OPT_NUM_DMOD_SELECTOR, Misc.getNegativeHighlightColor(), 256, 48, 1, maxAddDMods, ValueDisplayMode.VALUE, RANDOM_DMOD_SELECTOR_TOOLTIP);
